...
var data=[];
var datacltshow = [];
var receivedata ='start';
var invoicearray = [];
var orderarray = [];
var spec_function=[];

var plate_spec=[];
var plate_spec_option = [];
...
var operatorarray =['+','-','*','/','%','(',')'];
var mid_fuc=['unit','price','quantity'];
var paperarray=[];
var array_paper=[];

function f() {

    var r=math.eval('5+(6*7)');
    console.log(r);
...
router.get('/start/:id', function(req, res, next) {
    ...
    //     var dbo = db.db("mydb");
    //     //var myobj = { name: "Company Inc", address: "Highway 37" };
    //     dbo.collection("functions").find({}).toArray(function(err, result) {
    //         if (err) throw err;
    //         for(var i=0;i<result.length;i++) {
    //
    //             if(result[i]._id==req.params.id) {
...
router.get('/number/:id', function(req, res, next) {
    ...
    console.log('result  '+math.eval(responseText));
...
router.post('/print2',function (req,res,next) {
    ...
    if(type == 'textfield' || type == 'number' ||  type == 'date') {
        c = 1;
        o=0;
        u=0;
        p=0;
    ...
    if(req.body.check2==undefined)
        ppr=0;
    else
        ppr=1;
    if(c==0) {
        parsed += '<specification>#' + spec + '#<plate>#' + plt + '#</plate>#'+'#<paper>#' +ppr+ '#</paper>#<type>#' + type + '#';
    }
    else
    {
        parsed += '<specification>#' + spec + '#<plate>#' + plt + '#</plate>#' + '#<paper>#' +ppr+ '#</paper>#<type>#' + type + '#<option>#'+o+'#<price>#'+p+'#<unit>#'+p+'</unit>#</price>#</option>#</type>#</specification>#';
    ...
    var rdata='Specification :' + spec +'  Type :'+type+'  Plate:'+plt+'  Paper'+ppr;


    costestimationdata.push(rdata);
    // prdctname=productname;
    // aftername = 1;
    // console.log(productname);
    // costestimationdata=[];
    ...
    res.render('index',{s2:1,c:c,typearray:typearray,data:costestimationdata,speci:spec});
})
...
router.post('/print4',function (req,res,next) {
    ...
    var optn=req.body.option;
    var prce=req.body.price;
    var unt=req.body.unit;
    ...
    if(optn=="")
    {
        ...
        parsed += '<option>#' + optn + '#<price>#' + prce + '#<unit>#' + unt + '#</unit>#</price>#</option>#';
        parsed+='#</type>#<function>#';
    ...
    res.render('index',{fn:1,data:costestimationdata,typearray:typearray,operatorarray:operatorarray,mid_fuc:mid_fuc});
})


router.get('/print6',function (req,res,next) {
    parsed+='#</final_function>#</product>#';
    console.log('parsed :'+parsed);
    //
    mongo.connect(url, function(err, db) {
        if (err) throw err;
        var dbo = db.db("mydb");
        var myobj ={ _id: p_name, name:parsed};
        //var myobj = { name: "Company Inc", address: "Highway 37" };
...
router.post('/print5',function (req,res,next) {
    ...
    var rdata='Option :' + optn +'  Price :'+prce+'  Unit:'+unt;
...
router.post('/gntfnc',function (req,res,next) {
...
router.post('/gntfnc2',function (req,res,next) {
    ...
    parsed+='#<final_function>#';
    console.log(spec_function);
...
var fncarray='';
router.post('/gntfnc3',function (req,res,next) {
    ...
    res.render('index',{fn:1,operatorarray:operatorarray,specarray:spec_function,fnc:fnc,mid_fuc:mid_fuc});
})

router.post('/gntfnc4',function (req,res,next) {
...
router.post('/gntfnc5',function (req,res,next) {

    var optr=req.body.optr;
    fncarray+='#'+optr+'#';
    parsed+='#'+optr+'#';
...
router.get('/gntfnc6',function (req,res,next) {
    ...
    res.render('index',{s:1,typearray:typearray});
})

router.get('/gntfnc11',function (req,res,next) {

    var editfun=parsed.split('#');
    ...
    for(var i=editfun.length-1;i>=0;i--)
    {
        if(editfun[i]=='<function>')
        {
            editfun.splice(i,count+1);
            break;
    ...
    for(var j=0;j<editfun.length;j++)
    {
        newparsed+=editfun[j]+'#';
    ...
    console.log('edit  '+editfun);
    console.log('newparsed  '+parsed);
...
router.post('/gntfnc8',function (req,res,next) {
    var num=req.body.number;
    fncarray+='#'+num+'#';
    ...
    res.render('index',{gntfnc2:1,operatorarray:operatorarray,specarray:spec_function,fnc:fnc,mid_fuc:mid_fuc});
})
router.post('/gntfnc9',function (req,res,next) {
    var spec=req.body.spec;
    // fncarray+='#'+num+'#';
...
router.post('/gntfnc10',function (req,res,next) {
...
function choicePaper(paper,q) {

    console.log('paper_cost' + paper);
    ...
    for (var i = 0; i < paper.length; i++) {
        ...
        for (var j = 0; j < array_paper.length; j++) {
            if (array_paper[j] == '<paper>') {
                ...
                total_page = Math.ceil(q / num_of_print_per_page);

                if (total_page < max) {
                    max = total_page;
                    paper_name = array_paper[j + 1];
        ...
        console.log('paper for ' + a[0] + '&&&&' + b[0] + paper_name);
        console.log('max  ' + max);
        console.log('Paper Price :' + ppr_price);
        console.log('PlateName  :' +
            Plate(l_plt, w_plt,paper2));
    }

    // choicePlate(l_plt,w_plt,paper2);
}


function choicepaper2(paper,page) {
    ...
    for (var i = 0; i < paper.length; i++) {
        ...
        for (var j = 0; j < array_paper.length; j++) {
            if (array_paper[j] == '<paper>') {
                ...
                // row2=arr[j+2]/w;
                num_of_print_per_page = row * column;
                total_page = Math.ceil(page / num_of_print_per_page);
        ...
        // console.log('max  ' + max);
        // console.log('Paper Price :' + ppr_price);
        // console.log('PlateName  :' + choicePlate(l_plt, w_plt,paper));
    }
...
function choicePlate(l,w,paper)
{

    console.log('Paper---------in plt'+paper);
            ...
            // console.log("w :" + w);
            //
            // console.log('plate_spec'+plate_spec);
            // console.log('plate_spec_option'+plate_spec_option);
    var page;

    for(var i=0;i<plate_spec.length;i++)
    {
        if(plate_spec[i]=='Pagenumber')
        {
            // plate_for_page=Math.ceil(plate_spec_option[i]/num_of_print_per_plate);
            page=plate_spec_option[i];

            // plate_spec_option[i]=plate_for_page;
    ...
    var totalPage=choicepaper2(paper,page);
    ...
    console.log('total page '+totalPage);
            ...
            var platename;
            // pr=arr[3];
            for(var j=0;j<array_plate.length;j++) {
                if (array_plate[j] == '<plate>') {

                    // console.log("price"+pr);
                    // console.log("j value"+j);
                    // console.log("plt_lnth"+arr[j+2]);
                    // console.log("plt_wdth"+arr[j+5]);
                    row1 = parseInt((array_plate[j + 3] / l), 10);
                    column1 = parseInt((array_plate[j + 6] / w), 10);
                    row2 = parseInt((array_plate[j + 3] / w), 10);
                    column2 = parseInt((array_plate[j + 6] / l), 10);
                    ...
                    // plate_price=total_plate*arr[3];
                    if (num_of_print_per_plate > cmpr) {
                        cmpr = num_of_print_per_plate;
                        platename = array_plate[j + 1];
                    }
                    // if(arr[j+1]==l && arr[j+4]==w)
                    // {
                    //     console.log("calculation start");
                    ...
                    // }
                     ...
                     j=j+9;
                    // console.log('After Hasdhi  '+array_plate[j]);
                    // break;
            ...
            var plate_for_page=1;

            var total_plate=Math.ceil(totalPage/num_of_print_per_plate);

            console.log('print per plate'+num_of_print_per_plate);
            ...
            for(var i=0;i<plate_spec.length;i++)
            {
                if(plate_spec[i]=='Pagenumber')
                {
                    ...
                    plate_spec_option[i]=total_plate;
            ...
            console.log('new plate opt'+plate_spec_option);
            for (var k=0;k<plate_spec_option.length;k++)
            {
                console.log('num of plt'+num_of_plate);

                num_of_plate=num_of_plate*plate_spec_option[k];
            }

            console.log(num_of_plate);
...
router.post('/submit',function (req,res,next) {
    ...
    var text=[];
    text.push(req.body.number);

    var quantity=req.body.qtn;
    console.log('number '+text);

    var l=req.body.number[0];
    var w=req.body.number[1];
    // var w=text[1];

    console.log('l '+l);
    console.log('w '+w);
    console.log('qtn  '+quantity);
    choicePaper(l,w,quantity);
...
router.post('/datasubmit', function(req, res, next) {
    // if(!sess.email) {
    //     res.redirect('/');
    // }
    // console.log(name);
    ...
    mongo.connect(url, function(err, db) {
        ...
        for(var i=0;i<req.body.number.length;i++)
            number.push(req.body.number[i]);
        console.log('lnnt  '+number.length);
        ...
        dbo.collection("products").find({}).toArray(function(err, result) {
            ...
            for(var i=0;i<result.length;i++)
            {
                ...
                if(result[i]._id==p_name) {
                    dat = result[i].name;//taking all the data of that project
                    db.close();
                    //console.log(dat);
                    var arr=dat.split("#");
                    console.log('dhukse');
                    var checkflag=0,textflag=0,dropflag=0,radioflag=0,dateflag=0,numberflag=0,plateflag=0,lnth=0,wdth=0,paperflag=0;// checkflag = checkbox, dropflag = dropdown,textflag=textbox
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        // }
                            ...
                            if (checkflag == 1 ) {
                                ...
                                if(plateflag==1) {
                                    for (var j = dropcounter; j < check.length; j++) {
                                        console.log("drop array: " + check[j]);
                                        if (check[j] == dta) {
                                            l = check[j];

                                            console.log('MultiPlate Cost Started');
                                            plate_spec.push(spec);
                                            plate_spec_option.push(check[j]);
                                ...
                                else if(paperflag==1)
                                {
                                    ...
                                    console.log('hello from paperflag');
                                    ...
                                    for (var j = dropcounter; j < drop.length; j++) {
                                        ...
                                        if (a[1]==dta) {
                                            ...
                                            var x=spec+'$'+l;
                                            console.log('x------'+x);
                                            papercost_final.push(drop[j]);
                                            console.log('ppr cost final part'+papercost_final);
                                            dropcounter++;
                                            // lnth=0;
                                            paperflag=0;
                                            break;
...

                            {
                                ...
                                if(paperflag==1)
                                {
                                    console.log('spec b4 enter'+spec+number[numbercounter]);
                                    console.log('check num :'+number[numbercounter]);
                                    var x=spec+'$'+number[numbercounter];
                                    papercost_final.push(x);
                                    // paperflag=0;
                                    numbercounter++;
                                    paperflag=0;
                                ...
                                else if(plateflag==1)
                                {
                                    ...
                                    plate_spec.push(spec);
                                    plate_spec_option.push(number[numbercounter]);
                    ...
                    console.log('in function'+papercost_final);
                    choicePaper(papercost_final,quantity);
    ...
    for (var i = 0; i < arr.length; i++) {
        if ((arr[i] == '</product>') || (arr[i] == '</option>') || (arr[i] == '</price>') || (arr[i] == '</unit>')|| (arr[i] == '</specification>')) {
            ...
            // responseText = responseText + "<label>Type :</label><input type='text' name='text' value='" + arr[i + 1] + "' > ";
                ...
                responseText = responseText + "<select name='drop1'> <option>"+arr[i+1]+"</option>";

            for (var k = 0; k < typearray.length; k++) {
                if(arr[i+1]!=typearray[k])
                {
                responseText = responseText + "<label>Type :</label><option value='" + typearray[k] + "'>" + typearray[k] + "</option>";
                    }
                // responseText = responseText + "<label>Type :</label><select id='type'><option value='" + typearray[k] + "'></option> </select> ";
            }
            responseText = responseText + "</select>";
